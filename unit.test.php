...
class tests{
	static function is_equal_string($reference,$testCode){
		...
		if(!$passed) unit::output("Code return was unexpected result [RAW(".$result.") => Quoted(".htmlentities($result).")]");
		unit::result_output($resultElement,$passed);
		//unit::result_output("[".$reference."] ".__FUNCTION__." [".$testCode."]",$passed);
	}
}
...
class unit{
	...
	private static function convertto_html($string){
		...
		$string = str_replace("}}}","</pre>",$string);
		$string = preg_replace_callback("#<pre>(.*?)</pre>#is",function($m){return "<pre>".htmlentities($m[1])."</pre>";},$string);
		return $string;
	}
